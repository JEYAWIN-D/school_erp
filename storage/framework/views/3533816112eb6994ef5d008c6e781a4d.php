...
<div class="space-y-5">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div>
      ...
      <p class="page-subtitle"><?php echo e(now()->format('l, d M Y')); ?></p>
    </div>
    <div class="flex items-center gap-2 flex-wrap">
      <?php if($pendingLeaves > 0): ?>
      <a href="<?php echo e(route('attendance.leave')); ?>" class="btn btn-secondary btn-sm relative">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
        Leave Requests
        <span class="ml-0.5 inline-flex items-center justify-center w-4 h-4 bg-red-500 text-white text-[10px] font-bold rounded-full"><?php echo e($pendingLeaves); ?></span>
      </a>
      <?php endif; ?>
      <a href="<?php echo e(route('attendance.staff')); ?>" class="btn btn-secondary btn-sm">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
        Staff Attendance
      </a>
      <a href="<?php echo e(route('attendance.mark')); ?>" class="btn btn-primary btn-sm">
  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
    ...
    <div class="card border-l-4 <?php echo e($todayStats['total'] > 0 ? ($todayStats['pct'] >= 75 ? 'border-l-indigo-500' : 'border-l-red-500') : 'border-l-slate-300'); ?>">
      <div class="flex items-start justify-between">
        <div>
          <p class="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Today's Rate</p>
          <?php if($todayStats['total'] > 0): ?>
            <p class="text-2xl font-bold leading-none <?php echo e($todayStats['pct'] >= 75 ? 'text-indigo-700' : 'text-red-600'); ?>"
               style="font-family:'Plus Jakarta Sans',sans-serif;letter-spacing:-0.03em">
              <?php echo e($todayStats['pct']); ?>%
            </p>
          <?php else: ?>
            <p class="text-2xl font-bold text-slate-300 leading-none" style="font-family:'Plus Jakarta Sans',sans-serif">—</p>
          <?php endif; ?>
        </div>
        <div class="w-9 h-9 rounded-lg <?php echo e($todayStats['total'] > 0 ? ($todayStats['pct'] >= 75 ? 'bg-indigo-50' : 'bg-red-50') : 'bg-slate-100'); ?> flex items-center justify-center">
          <svg class="w-[18px] h-[18px] <?php echo e($todayStats['total'] > 0 ? ($todayStats['pct'] >= 75 ? 'text-indigo-600' : 'text-red-500') : 'text-slate-400'); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="card py-4">
    <div class="flex items-center justify-between mb-2.5">
      ...
      <span class="text-sm font-bold <?php echo e($todayStats['pct'] >= 75 ? 'text-emerald-700' : 'text-red-600'); ?>">
        <?php echo e($todayStats['pct']); ?>%
      </span>
    </div>
    <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
      <div class="h-2 rounded-full transition-all duration-500 <?php echo e($todayStats['pct'] >= 75 ? 'bg-emerald-500' : 'bg-red-500'); ?>"
           style="width: <?php echo e($todayStats['pct']); ?>%"></div>
    </div>
    <p class="text-xs text-slate-400 mt-2">
      <span class="font-semibold text-slate-600"><?php echo e($todayStats['present']); ?></span> present out of
      <span class="font-semibold text-slate-600"><?php echo e($todayStats['total']); ?></span> students marked today
    </p>
  </div>
  <?php endif; ?>

  
  <?php if($unmarkedClasses->count() > 0): ?>
  <div class="alert-warning">
    <div class="flex-shrink-0 mt-0.5">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
    </div>
    <div class="flex-1 min-w-0">
      <p class="font-semibold"><?php echo e($unmarkedClasses->count()); ?> class<?php echo e($unmarkedClasses->count() > 1 ? 'es have' : ' has'); ?> not marked attendance today</p>
      <div class="mt-2 flex flex-wrap gap-2">
        <?php $__currentLoopData = $unmarkedClasses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cls): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <a href="<?php echo e(route('attendance.mark', ['class_id' => $cls->id, 'date' => today()->toDateString()])); ?>"
  ...
  <div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
      ...
      <a href="<?php echo e(route($route)); ?>"
         class="card-flat flex items-center gap-3 py-3.5 px-4 border hover:shadow-sm hover:border-slate-300 transition-all group <?php echo e($color); ?>">
        ...
        <?php if($label === 'Leave Requests' && $pendingLeaves > 0): ?>
          <span class="inline-flex items-center justify-center w-5 h-5 bg-red-500 text-white text-[10px] rounded-full font-bold flex-shrink-0"><?php echo e($pendingLeaves); ?></span>
        <?php endif; ?>
      </a>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\school_erp - Copy\resources\views\attendance\index.blade.php ENDPATH**/ ?>