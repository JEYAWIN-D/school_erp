<?php $__env->startSection('title', 'Question Paper Templates'); ?>
...
<div class="space-y-6" x-data="{ showAdd: false }">
  ...
  <div x-show="showAdd" x-transition class="card">
    ...
    <form method="POST" action="<?php echo e(route('examinations.paper-templates.store')); ?>" class="space-y-5">
      <?php echo csrf_field(); ?>
      ...
      <div class="border-t border-slate-100 pt-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <?php $__currentLoopData = ['show_school_logo'=>'School Logo','show_school_name'=>'School Name','show_school_address'=>'Address','show_affiliation'=>'Affiliation No.']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k=>$label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <label class="flex items-center gap-2 text-sm text-slate-600">
            <input type="checkbox" name="<?php echo e($k); ?>" value="1" checked class="w-4 h-4 rounded"><?php echo e($label); ?>
    ...
    <div class="flex items-start justify-between">
      <div>
        <div class="flex items-center gap-2 mb-1">
          <h3 class="font-semibold text-slate-800"><?php echo e($t->name); ?></h3>
          <?php if($t->is_default): ?><span class="badge-green text-xs">Default</span><?php endif; ?>
          <?php if($t->exam_type): ?><span class="badge-slate text-xs capitalize"><?php echo e(str_replace('_',' ',$t->exam_type)); ?></span><?php endif; ?>
        </div>
        <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 mt-2">
          <span>Paper: <?php echo e($t->paper_size); ?></span>
          <span>Font: <?php echo e($t->font_size); ?>pt</span>
          <span>Numbering: <?php echo e(ucfirst($t->question_numbering)); ?></span>
          <?php if($t->show_school_name): ?><span class="text-green-600">School Name ✓</span><?php endif; ?>
          <?php if($t->show_affiliation): ?><span class="text-green-600">Affiliation ✓</span><?php endif; ?>
          <?php if($t->show_marks_per_question): ?><span class="text-green-600">Marks/Question ✓</span><?php endif; ?>
          <?php if($t->watermark_text): ?><span class="text-amber-600">Watermark: <?php echo e($t->watermark_text); ?></span><?php endif; ?>
        </div>
        <?php if($t->general_instructions): ?>
        <p class="text-xs text-slate-400 mt-2 italic max-w-xl line-clamp-2"><?php echo e($t->general_instructions); ?></p>
        <?php endif; ?>
      </div>
      <form method="POST" action="<?php echo e(route('examinations.paper-templates.delete', $t->id)); ?>" onsubmit="return confirm('Delete this template?')">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\school_erp - Copy\resources\views\examinations\question-paper-templates.blade.php ENDPATH**/ ?>