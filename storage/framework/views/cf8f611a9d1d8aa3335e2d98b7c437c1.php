...
<div class="space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <p class="page-subtitle"><?php echo e($stats['residents']); ?> residents across <?php echo e($hostels->count()); ?> hostel(s)</p>
    </div>
    <a href="<?php echo e(route('hostel.allotment')); ?>" class="btn btn-primary self-start sm:self-auto">
  ...
  <div class="rounded-xl border border-red-200 bg-red-50 p-4 flex items-center gap-3">
    ...
    <p class="text-sm font-semibold text-red-800">
      <?php echo e($overdueOutpasses); ?> outpass(es) overdue — students have not returned as scheduled.
      <a href="<?php echo e(route('hostel.overdue-outpasses')); ?>" class="underline ml-1">Review →</a>
    </p>
  </div>
  <?php endif; ?>
  ...
  <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 flex items-center gap-3">
    ...
    <p class="text-sm font-semibold text-amber-800">
      <?php echo e($pendingComplaints); ?> open complaint(s) awaiting resolution.
      <a href="<?php echo e(route('hostel.complaints')); ?>" class="underline ml-1">Resolve →</a>
    </p>
  </div>
  <?php endif; ?>
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <div class="card">
      ...
      <p class="stat-number"><?php echo e($stats['totalRooms']); ?></p>
      <p class="text-sm text-slate-500">Total Rooms</p>
    </div>
    <a href="<?php echo e(route('hostel.occupancy')); ?>" class="card hover:shadow-card-md transition">
      ...
      <p class="stat-number"><?php echo e($stats['occupiedRooms']); ?></p>
    ...
    <div class="card">
      ...
      <p class="stat-number text-green-600"><?php echo e($stats['availableRooms']); ?></p>
      <p class="text-sm text-slate-500">Available Rooms</p>
    </div>
    <a href="<?php echo e(route('hostel.room-students')); ?>" class="card hover:shadow-card-md transition">
      ...
      <p class="stat-number"><?php echo e($stats['residents']); ?></p>
      <p class="text-sm text-slate-500">Residents</p>
    </a>
  </div>

  
  <?php if($stats['totalRooms'] > 0): ?>
  <div class="card">
    <div class="flex items-center justify-between mb-2">
      <h3 class="font-semibold text-slate-700">Overall Occupancy</h3>
      <span class="text-lg font-bold <?php echo e($stats['occupancyPct'] >= 90 ? 'text-red-600' : ($stats['occupancyPct'] >= 70 ? 'text-amber-600' : 'text-green-600')); ?>"><?php echo e($stats['occupancyPct']); ?>%</span>
    </div>
    <div class="w-full bg-slate-100 rounded-full h-3">
      <div class="h-3 rounded-full transition-all <?php echo e($stats['occupancyPct'] >= 90 ? 'bg-gradient-to-r from-red-500 to-rose-400' : ($stats['occupancyPct'] >= 70 ? 'bg-gradient-to-r from-amber-500 to-orange-400' : 'bg-gradient-to-r from-green-500 to-emerald-400')); ?>"
           style="width:<?php echo e(min(100,$stats['occupancyPct'])); ?>%"></div>
    </div>
    <p class="text-xs text-slate-400 mt-1.5"><?php echo e($stats['occupiedRooms']); ?> of <?php echo e($stats['totalRooms']); ?> rooms occupied &mdash; <?php echo e($stats['availableRooms']); ?> available</p>
  </div>
  <?php endif; ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div class="card">
        <div class="flex items-center gap-3 mb-3">
          ...
          <div>
            <p class="font-semibold text-slate-800"><?php echo e($h->name); ?></p>
            <span class="badge-slate capitalize text-xs"><?php echo e($h->type); ?></span>
          </div>
        </div>
        <dl class="text-sm space-y-1.5 mb-3">
          <div class="flex justify-between"><dt class="text-slate-400">Warden</dt><dd class="font-medium text-slate-700"><?php echo e($h->warden_name ?? '—'); ?></dd></div>
          <div class="flex justify-between"><dt class="text-slate-400">Capacity</dt><dd class="font-medium text-slate-700"><?php echo e($h->total_capacity); ?></dd></div>
          <div class="flex justify-between"><dt class="text-slate-400">Rooms</dt><dd class="font-medium text-slate-700"><?php echo e($h->rooms_count); ?> (<?php echo e($h->occupied_count); ?> occupied)</dd></div>
        </dl>
        <div class="w-full bg-slate-100 rounded-full h-2 mb-1">
          <div class="h-2 rounded-full transition-all" style="width:<?php echo e(min(100,$occ)); ?>%;background:<?php echo e($occColor); ?>;"></div>
        </div>
        <p class="text-xs text-slate-400 mb-3"><?php echo e($occ); ?>% occupancy</p>
        <div class="flex gap-2 mt-3">
          <a href="<?php echo e(route('hostel.rooms', ['hostel_id' => $h->id])); ?>" class="btn btn-ghost btn-sm flex-1 justify-center">View Rooms</a>
          <button type="button" onclick="openEditHostel(<?php echo e($h->id); ?>, '<?php echo e(addslashes($h->name)); ?>', '<?php echo e($h->type); ?>', <?php echo e($h->total_capacity ?? 0); ?>, '<?php echo e(addslashes($h->address ?? '')); ?>', '<?php echo e(addslashes($h->contact ?? '')); ?>')"
  ...
  <div id="addHostelModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
      ...
      <form method="POST" action="<?php echo e(route('hostel.building.store')); ?>" class="p-4 space-y-3">
        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\school_erp - Copy\resources\views\hostel\index.blade.php ENDPATH**/ ?>