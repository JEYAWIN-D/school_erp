<?php $__env->startSection('title', 'Mess Management'); ?>
...
<div class="space-y-6" x-data="{
  editDay: null,
  editHostel: null,
  allMenus: <?php echo e(json_encode($allMenus)); ?>,
  get currentMenu() {
  ...
  <div class="grid grid-cols-2 lg:grid-cols-<?php echo e(count($hostels)); ?> gap-4">
    ...
    <div class="card text-center py-5">
      ...
      <p class="text-xs text-slate-400 mt-1">Capacity: <?php echo e($h->total_capacity); ?></p>
      <span class="badge-green mt-2"><?php echo e(ucfirst($h->type)); ?></span>
    </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  </div>

  <?php $__currentLoopData = $hostels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $h): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
  <?php $menus = $h->meal_menus ? json_decode($h->meal_menus, true) : []; ?>
  <div class="card">
    <h3 class="font-semibold text-slate-700 mb-4">Weekly Menu — <?php echo e($h->name); ?></h3>
    <div class="overflow-x-auto">
      <table class="min-w-full text-sm">
        <thead>
          <tr class="text-left text-slate-500 border-b border-slate-100">
            <?php $__currentLoopData = ['Day','Breakfast','Lunch','Snacks','Dinner','']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $col): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <th class="pb-2 pr-4 font-medium"><?php echo e($col); ?></th>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          </tr>
        </thead>
        <tbody>
          ...
          <tr class="border-b border-slate-50 hover:bg-slate-50">
            <td class="py-2 pr-4 font-medium text-slate-700"><?php echo e($day); ?></td>
            <td class="py-2 pr-4 text-slate-500"><?php echo e($menu['breakfast'] ?? 'Not set'); ?></td>
            <td class="py-2 pr-4 text-slate-500"><?php echo e($menu['lunch'] ?? 'Not set'); ?></td>
            <td class="py-2 pr-4 text-slate-500"><?php echo e($menu['snacks'] ?? 'Not set'); ?></td>
            <td class="py-2 pr-4 text-slate-500"><?php echo e($menu['dinner'] ?? 'Not set'); ?></td>
            <td class="py-2">
              <button @click="editDay = '<?php echo e($day); ?>'; editHostel = <?php echo e($h->id); ?>" class="text-xs text-indigo-600 hover:underline">Edit</button>
            </td>
          </tr>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </tbody>
  ...
  <div x-show="editDay !== null" x-transition class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
    <div class="modal-box max-w-md w-full" @click.outside="editDay = null">
      ...
      <form method="POST" action="<?php echo e(route('hostel.mess.menu')); ?>" class="space-y-3">
        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\school_erp - Copy\resources\views\hostel\mess.blade.php ENDPATH**/ ?>