...
<div class="space-y-5" x-data="{
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div>
      ...
      <p class="page-subtitle">
        ...
        <span class="font-semibold text-slate-700"><?php echo e($students->total()); ?></span>
        <?php echo e($students->total() === 1 ? 'student' : 'students'); ?>

      </p>
    </div>
    <div class="flex items-center gap-2 flex-wrap">
      <a href="<?php echo e(route('students.directory')); ?>" class="btn btn-secondary btn-sm">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
        Directory
      </a>
      <a href="<?php echo e(route('students.tc-register')); ?>" class="btn btn-secondary btn-sm">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
        TC Register
      </a>
      <a href="<?php echo e(route('students.export', request()->query())); ?>" class="btn btn-secondary btn-sm">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
        Export
      </a>
      <a href="<?php echo e(route('students.import')); ?>" class="btn btn-secondary btn-sm">
    ...
    <div class="filter-bar">
      ...
      <div class="relative flex-1 min-w-48">
        ...
        <input type="text" name="search" value="<?php echo e(request('search')); ?>"
               ...
               class="input w-full"
               style="padding-left: 2.25rem; padding-right: <?php echo e(request('search') ? '2rem' : '0.75rem'); ?>">
        <?php if(request('search')): ?>
          <a href="<?php echo e(route('students.index', request()->except('search'))); ?>"
      ...
      <select name="section" class="select w-32">
        ...
        <?php $__currentLoopData = $sectionNames; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $secName): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <option value="<?php echo e($secName); ?>" <?php if(strtoupper(trim(preg_replace('/^section\s*/i', '', request('section', '')))) === strtoupper($secName)): echo 'selected'; endif; ?>>Section <?php echo e($secName); ?></option>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </select>

      <select name="status" class="select w-32">
        <option value="active"      <?php if(request('status', 'active') === 'active'): echo 'selected'; endif; ?>>Active</option>
        <option value="inactive"    <?php if(request('status') === 'inactive'): echo 'selected'; endif; ?>>Inactive</option>
        <option value="left"        <?php if(request('status') === 'left'): echo 'selected'; endif; ?>>Left</option>
        <option value="transferred" <?php if(request('status') === 'transferred'): echo 'selected'; endif; ?>>Transferred</option>
        <option value="all"         <?php if(request('status') === 'all'): echo 'selected'; endif; ?>>All Status</option>
      </select>

      <select name="sort" class="select w-40">
        <option value="latest"    <?php if(request('sort', 'latest') === 'latest'): echo 'selected'; endif; ?>>Latest First</option>
        ...
        <option value="roll_asc"  <?php if(request('sort') === 'roll_asc'): echo 'selected'; endif; ?>>By Roll No.</option>
        <option value="adm_asc"   <?php if(request('sort') === 'adm_asc'): echo 'selected'; endif; ?>>By Adm. No.</option>
      </select>
      ...
      <?php if(request()->hasAny(['search','class_id','section','sort']) || (request('status') && request('status') !== 'active')): ?>
        <a href="<?php echo e(route('students.index')); ?>" class="btn btn-ghost btn-sm text-slate-500">
          Clear
        </a>
      <?php endif; ?>
    </div>
    ...
    <table class="w-full">
      ...
      <tbody>
        <?php $__empty_1 = true; $__currentLoopData = $students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          ...
          ?>
          <tr class="tr cursor-pointer" @click="openQuickView(<?php echo e($qv); ?>)">
            <td class="td">
              <span class="font-mono text-xs font-semibold text-indigo-600"><?php echo e($student->admission_number); ?></span>
            </td>
            <td class="td">
              <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 rounded-full bg-indigo-50 flex items-center justify-center flex-shrink-0 overflow-hidden ring-1 ring-slate-200">
                  <?php if($student->photo): ?>
                    <img src="<?php echo e(asset('storage/'.$student->photo)); ?>" class="w-full h-full object-cover">
                  <?php else: ?>
                    <span class="text-indigo-600 text-[10px] font-bold"><?php echo e(strtoupper(substr($student->first_name, 0, 1) . substr($student->last_name, 0, 1))); ?></span>
                  <?php endif; ?>
                </div>
                <div>
                  <p class="font-semibold text-slate-800 text-sm leading-tight"><?php echo e($student->full_name); ?></p>
                  <p class="text-xs text-slate-400 mt-0.5"><?php echo e($student->dob?->format('d M Y')); ?></p>
                </div>
              </div>
            </td>
            <td class="td">
              <?php if($student->currentEnrollment): ?>
                <p class="font-semibold text-slate-800 text-sm leading-tight"><?php echo e($student->currentEnrollment->class?->name); ?></p>
                <p class="text-xs text-slate-400"><?php echo e($student->currentEnrollment->section?->name); ?></p>
              <?php else: ?>
                <span class="text-slate-300">—</span>
              <?php endif; ?>
            </td>
            <td class="td font-mono text-sm text-slate-600"><?php echo e($student->currentEnrollment?->roll_number ?? '—'); ?></td>
            <td class="td text-slate-700"><?php echo e($student->father_name ?: '—'); ?></td>
            <td class="td font-mono text-sm text-slate-600"><?php echo e($student->father_mobile ?? $student->mobile ?? '—'); ?></td>
            <td class="td">
              <span class="<?php echo e($student->status === 'active' ? 'badge-green' : ($student->status === 'left' ? 'badge-rose' : 'badge-slate')); ?>">
            ...
            <td class="td text-right" @click.stop>
              <div class="flex items-center justify-end gap-0.5">
                <a href="<?php echo e(route('students.show', $student->id)); ?>" class="btn-icon" title="View student">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                </a>
                <a href="<?php echo e(route('students.edit', $student->id)); ?>" class="btn-icon" title="Edit student">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                </a>
                <a href="<?php echo e(route('students.tc.form', $student->id)); ?>" class="btn-icon" title="Transfer certificate">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </a>
              </div>
            </td>
  ...
  <?php if($students->hasPages()): ?>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-500">
      <span>
        Showing <span class="font-semibold text-slate-700"><?php echo e($students->firstItem()); ?></span>–<span class="font-semibold text-slate-700"><?php echo e($students->lastItem()); ?></span>
        of <span class="font-semibold text-slate-700"><?php echo e($students->total()); ?></span> students
      </span>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\school_erp - Copy\resources\views\students\index.blade.php ENDPATH**/ ?>