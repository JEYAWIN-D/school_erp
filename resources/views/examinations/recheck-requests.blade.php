@extends('layouts.app')
@section('title','Marks Recheck Requests')
@section('content')
<div class="space-y-6">
    ...
    <div class="flex flex-wrap gap-3 items-end">
      ...
      <div>
        ...
        <select name="status" class="select w-36">
          ...
          @foreach(['pending'=>'Pending','under_review'=>'Under Review','revised'=>'Revised','rejected'=>'Rejected'] as $v=>$l)
  ...
  <div class="card overflow-hidden">
    <table class="min-w-full text-sm">
      <thead class="bg-slate-50 border-b border-slate-100"><tr>
        @foreach(['Student','Exam','Subject','Marks Before','Marks After','Reason','Status','Actions'] as $h)
      ...
      <tbody class="divide-y divide-slate-100">
        ...
        <tr class="hover:bg-slate-50">
          <td class="px-4 py-3">
            <p class="font-medium text-slate-800">{{ $r->student?->full_name }}</p>
            <p class="text-xs text-slate-400">{{ $r->student?->admission_number }}</p>
          </td>
          <td class="px-4 py-3 text-xs text-slate-600">{{ $r->exam?->name }}</td>
          <td class="px-4 py-3 text-xs text-slate-600">{{ $r->examSchedule?->subject?->name }}</td>
          <td class="px-4 py-3 font-mono text-slate-700">{{ $r->marks_before ?? '—' }}</td>
          <td class="px-4 py-3 font-mono {{ $r->marks_after ? 'text-green-600 font-semibold' : 'text-slate-300' }}">
            {{ $r->marks_after ?? '—' }}
          </td>
          <td class="px-4 py-3 text-xs text-slate-500 max-w-xs">{{ Str::limit($r->reason, 50) ?? '—' }}</td>
          <td class="px-4 py-3">
            @php $statusColors = ['pending'=>'badge-amber','under_review'=>'badge-blue','revised'=>'badge-green','rejected'=>'badge-red']; @endphp
            <span class="{{ $statusColors[$r->status] ?? 'badge-slate' }} text-xs">{{ str_replace('_',' ',ucfirst($r->status)) }}</span>
          </td>
          <td class="px-4 py-3" x-data="{process:false}">
            @if(in_array($r->status, ['pending','under_review']))
            ...
            <div x-show="process" x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="process=false">
              <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4" @click.stop>
                <h3 class="font-semibold">Process Recheck — {{ $r->student?->first_name }}</h3>
                <p class="text-sm text-slate-500">{{ $r->examSchedule?->subject?->name }} | Original: {{ $r->marks_before }}</p>
                <form method="POST" action="{{ route('examinations.recheck-requests.process', $r->id) }}" class="space-y-3">
                  @csrf
                  <div>
                  ...
                  <div>
                    ...
                    <input type="number" name="marks_after" class="input" min="0" max="{{ $r->examSchedule?->max_marks }}" step="0.5" placeholder="New marks">
                  </div>
                  <div>
            ...
            @elseif($r->admin_remarks)
            <span class="text-xs text-slate-400">{{ Str::limit($r->admin_remarks, 40) }}</span>
            @endif
          </td>
        </tr>
        @empty
    ...
    <div class="px-4 pb-3">{{ $requests->links() }}</div>
    @endif
  </div>
</div>
...
<div x-data="{open:false}" @open-modal.window="if($event.detail==='new-recheck')open=true"
  ...
  <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6 z-10 space-y-4">
    ...
    <form method="POST" action="{{ route('examinations.recheck-requests.store') }}" class="space-y-3">
      @csrf
      <div>
