...
<p>
  ...
  bearing Employee Number <strong>{{ $employee->employee_number }}</strong>, was employed with
  ...
  <strong>{{ \Carbon\Carbon::parse($employee->joining_date)->format('d F Y') }}</strong>
  to <strong>{{ now()->format('d F Y') }}</strong>.
</p>

<p>
  He/She has been relieved from the services of this institution with effect from
  <strong>{{ now()->format('d F Y') }}</strong>, upon his/her request. All company property
  has been returned and no dues are outstanding.
</p>

<p>
  We wish {{ $employee->first_name }} the very best in all future endeavours.
</p>
