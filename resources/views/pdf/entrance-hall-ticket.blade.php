...
<div class="page">
  ...
  <div class="info-grid">
    <div class="info-row">
      ...
      <div class="info-value"><strong>{{ $enquiry->enquiry_number }}</strong></div>
    </div>
    <div class="info-row">
      <div class="info-label">Candidate Name</div>
      <div class="info-value"><strong>{{ strtoupper($enquiry->student_name) }}</strong></div>
    </div>
    <div class="info-row">
      <div class="info-label">Date of Birth</div>
      <div class="info-value">{{ $enquiry->dob ? \Carbon\Carbon::parse($enquiry->dob)->format('d M Y') : '—' }}</div>
    </div>
    <div class="info-row">
      <div class="info-label">Class Applied For</div>
      <div class="info-value">{{ $enquiry->class?->name ?? '—' }}</div>
    </div>
    <div class="info-row">
      <div class="info-label">Parent / Guardian</div>
      <div class="info-value">{{ $enquiry->parent_name }}</div>
    </div>
    <div class="info-row">
      <div class="info-label">Contact</div>
      <div class="info-value">{{ $enquiry->parent_mobile }}</div>
    </div>
    <div class="info-row">
      <div class="info-label">Academic Year</div>
      <div class="info-value">{{ $enquiry->academicYear?->name ?? '—' }}</div>
    </div>
  </div>
  ...
  <div class="test-box">
    <div class="test-date">{{ $enquiry->entrance_test_date->format('l, d F Y') }}</div>
    @if($enquiry->entrance_test_time)
      <div class="test-time">Time: {{ \Carbon\Carbon::createFromFormat('H:i', $enquiry->entrance_test_time)->format('h:i A') }}</div>
    @endif
    @if($enquiry->entrance_test_venue)
      <div class="test-venue">Venue: {{ $enquiry->entrance_test_venue }}</div>
    @endif
    @if($enquiry->entrance_test_invigilator)
      <div class="test-venue" style="margin-top:4px;">Invigilator: {{ $enquiry->entrance_test_invigilator }}</div>
    @endif
  </div>
...
<div class="watermark">Generated: {{ now()->format('d M Y, h:i A') }}</div>
</body>
</html>
