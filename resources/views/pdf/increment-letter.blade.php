...
<p>
  ...
  <strong>{{ $school->school_name ?? $school->name ?? 'our institution' }}</strong>,
...
<table class="info-table">
  ...
  <tr><td>Employee Number</td><td>{{ $employee->employee_number }}</td></tr>
  <tr><td>Designation</td><td>{{ $employee->designation ?? '—' }}</td></tr>
  <tr><td>Department</td><td>{{ $employee->department ?? '—' }}</td></tr>
  <tr><td>Date of Joining</td><td>{{ \Carbon\Carbon::parse($employee->joining_date)->format('d M Y') }}</td></tr>
  <tr><td>Current Basic Salary</td><td>₹{{ number_format($employee->basic_salary ?? 0, 2) }} per month</td></tr>
  <tr><td>Revised Basic Salary</td><td>₹{{ number_format($newSalary, 2) }} per month</td></tr>
  <tr><td>Effective From</td><td>{{ now()->addMonth()->startOfMonth()->format('d M Y') }}</td></tr>
</table>

<div class="highlight">
  Net Salary Increment: ₹{{ number_format($newSalary - ($employee->basic_salary ?? 0), 2) }} per month
</div>
...
<div class="sig">
  ...
  <div class="sig-box">Principal / Director<br>{{ $school->school_name ?? 'School Name' }}</div>
</div>
</body>
</html>
