...
<div class="ref-row">
  <span>Ref: WRN/{{ $student->admission_number }}/{{ now()->format('Y') }}</span>
  <span>Date: {{ now()->format('d F Y') }}</span>
</div>

<div class="salutation">
  To,<br>
  <strong>The Parent/Guardian of {{ $student->full_name }}</strong><br>
  @if($student->residential_address){{ $student->residential_address }}<br>@endif
</div>

<div class="body-text">
  <p>Dear Parent/Guardian,</p>

  <p>We regret to inform you that your ward <span class="highlight">{{ $student->full_name }}</span> (Admission No: <strong>{{ $student->admission_number }}</strong>), studying in <span class="highlight">{{ $student->currentEnrollment?->class?->name }} {{ $student->currentEnrollment?->section?->name }}</span>, has been involved in a disciplinary incident as detailed below:</p>
</div>

<table class="details">
  <tr><td>Student Name</td><td>{{ $student->full_name }}</td></tr>
  <tr><td>Admission Number</td><td>{{ $student->admission_number }}</td></tr>
  <tr><td>Class</td><td>{{ $student->currentEnrollment?->class?->name }} {{ $student->currentEnrollment?->section?->name }}</td></tr>
  <tr><td>Incident Date</td><td>{{ $incident->incident_date?->format('d F Y') }}</td></tr>
  <tr><td>Incident Type</td><td>{{ ucfirst(str_replace('_', ' ', $incident->incident_type)) }}</td></tr>
  <tr><td>Description</td><td>{{ $incident->description }}</td></tr>
  @if($incident->action_taken)
  <tr><td>Action Taken</td><td>{{ $incident->action_taken }}</td></tr>
  @endif
</table>
...
<div class="ack">
  <p><strong>Acknowledgement:</strong> I, the parent/guardian of {{ $student->full_name }}, have received this warning letter and have noted its contents.</p>
  <br>
  <table width="100%"><tr>
