...
<div class="header">
  ...
  @if($school?->gstin)
  <div class="school-sub">GSTIN: {{ $school->gstin }}</div>
  @endif
</div>
...
<table>
  ...
  @endphp
  @if($feeHead?->gst_applicable && $feeHead->gst_percent > 0)
  <tr><td>Taxable Amount</td><td>₹{{ number_format($baseAmount, 2) }}</td></tr>
  <tr><td>GST ({{ $feeHead->gst_percent }}% — {{ strtoupper($feeHead->gst_type ?? 'exclusive') }})</td><td>₹{{ number_format($gstAmt, 2) }}</td></tr>
  @else
  <tr><td>Amount</td><td>₹{{ number_format($payment->amount, 2) }}</td></tr>
  @endif
  @if($payment->late_fee > 0)
  <tr><td>Late Fee</td><td>₹{{ number_format($payment->late_fee, 2) }}</td></tr>
  @endif
  @if($payment->discount > 0)
  <tr><td>Discount</td><td>- ₹{{ number_format($payment->discount, 2) }}</td></tr>
  @endif
  <tr class="total-row"><td>Total Paid</td><td>₹{{ number_format($payment->total_paid, 2) }}</td></tr>
</table>
...
<div class="footer">This is a computer generated receipt. No signature required. | Collected by: {{ $payment->collectedBy?->name ?? '—' }}</div>
</body>
</html>
