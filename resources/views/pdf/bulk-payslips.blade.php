...
<div class="slip">
  <div class="header">
    <div class="school-name">{{ $school->name ?? $school->school_name ?? 'SCHOOL NAME' }}</div>
    <div style="font-size:9px;color:#555;margin-top:2px;">Pay Slip — {{ \Carbon\Carbon::createFromFormat('Y-m', $year . '-' . $month)->format('F Y') }}</div>
  </div>

  <div class="info-grid">
    <div class="info-box"><div class="info-label">Employee Name</div><div class="info-value">{{ $emp->first_name }} {{ $emp->last_name }}</div></div>
    <div class="info-box"><div class="info-label">Emp No</div><div class="info-value">{{ $emp->employee_number }}</div></div>
    <div class="info-box"><div class="info-label">Designation</div><div class="info-value">{{ $emp->designation ?? '—' }}</div></div>
    <div class="info-box"><div class="info-label">Department</div><div class="info-value">{{ $emp->department ?? '—' }}</div></div>
    <div class="info-box"><div class="info-label">Working / Present</div><div class="info-value">{{ $payroll->working_days ?? 26 }} / {{ $payroll->present_days ?? 26 }}</div></div>
  </div>
  ...
  <div class="cols">
    <div class="col">
      <table>
        ...
        <tbody>
          <tr><td>Basic Salary</td><td class="amount">{{ number_format($basic, 2) }}</td></tr>
          <tr><td>HRA</td><td class="amount">{{ number_format($hra, 2) }}</td></tr>
          <tr><td>Transport Allowance</td><td class="amount">{{ number_format($ta, 2) }}</td></tr>
          @if($other > 0)<tr><td>Other Allowances</td><td class="amount">{{ number_format($other, 2) }}</td></tr>@endif
          <tr class="total-row"><td>Gross Earnings</td><td class="amount">{{ number_format($gross, 2) }}</td></tr>
        </tbody>
      </table>
    </div>
    <div class="col">
      <table>
        ...
        <tbody>
          <tr><td>PF (Employee)</td><td class="amount">{{ number_format($pf, 2) }}</td></tr>
          @if($esi > 0)<tr><td>ESI (Employee)</td><td class="amount">{{ number_format($esi, 2) }}</td></tr>@endif
          @if($tds > 0)<tr><td>TDS</td><td class="amount">{{ number_format($tds, 2) }}</td></tr>@endif
          @if($otherDeductions > 0)<tr><td>Other Deductions</td><td class="amount">{{ number_format($otherDeductions, 2) }}</td></tr>@endif
  ...
  <div class="net-box">
    ...
    <div class="net-amount">₹{{ number_format($net, 2) }}</div>
  </div>
