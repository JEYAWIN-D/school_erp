...
<body>
<div class="header">
  <div class="school-name">{{ $school?->school_name ?? 'Library' }} — Book QR Labels</div>
  <div class="subtitle">Total: {{ $booksWithQr->count() }} labels | Generated: {{ now()->format('d M Y') }}</div>
</div>

<div class="labels-grid">
  @foreach($booksWithQr as $item)
  <div class="label">
    <div class="qr">{!! $item['qr'] !!}</div>
    <div class="info">
      <div class="accession">{{ $item['qrData'] }}</div>
      <div class="title">{{ \Illuminate\Support\Str::limit($item['book']->title, 35) }}</div>
      @if($item['book']->author)
        <div class="isbn">{{ \Illuminate\Support\Str::limit($item['book']->author, 20) }}</div>
      @endif
      @if($item['book']->isbn)
        <div class="isbn">ISBN: {{ $item['book']->isbn }}</div>
      @endif
    </div>
  </div>
  @endforeach
