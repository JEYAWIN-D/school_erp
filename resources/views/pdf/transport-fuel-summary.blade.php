...
<div class="header">
  ...
  <div class="meta">Month: {{ $monthLabel }} &nbsp;|&nbsp; Generated: {{ now()->format('d M Y') }}</div>
</div>

<div class="totals">
  <div class="total-box">
    <div class="total-val">{{ number_format($grandTotals['litres'],1) }} L</div>
    <div class="total-label">Total Fuel Consumed</div>
  </div>
  <div class="total-box" style="padding-left:6px;">
    <div class="total-val">₹{{ number_format($grandTotals['amount'],0) }}</div>
...
<table>
  ...
  <tbody>
    ...
    @php $efficiency = ($row->total_litres > 0 && $row->km_covered) ? round($row->km_covered/$row->total_litres,2) : null; @endphp
    <tr>
      <td>{{ $row->vehicle?->vehicle_number ?? ('Vehicle #'.$row->vehicle_id) }}</td>
      <td style="text-align:center;">{{ $row->fill_count }}</td>
      <td style="text-align:right;">{{ number_format($row->total_litres,1) }}</td>
      <td style="text-align:right;">{{ number_format($row->total_amount,0) }}</td>
      <td style="text-align:right;">{{ $row->km_covered ? number_format($row->km_covered,0) : '—' }}</td>
      <td style="text-align:center;">{{ $efficiency ? $efficiency.' km/L' : '—' }}</td>
    </tr>
    @endforeach
    <tr class="total-row">
      <td>Total</td>
      <td style="text-align:center;">{{ $fuelByVehicle->sum('fill_count') }}</td>
      <td style="text-align:right;">{{ number_format($grandTotals['litres'],1) }}</td>
      <td style="text-align:right;">{{ number_format($grandTotals['amount'],0) }}</td>
      <td>—</td><td>—</td>
    </tr>
  </tbody>
</table>

<footer>{{ $school?->school_name ?? 'School' }} — Fuel Expense Summary — {{ $monthLabel }}</footer>
</body>
</html>
