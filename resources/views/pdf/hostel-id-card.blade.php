...
<div class="card">
  <div class="header">
    ...
    <div style="font-size:7.5px; color:#93c5fd; margin-top:2px;">{{ $school['address'] ?? '' }}</div>
  </div>
  <div class="card-title">Hostel Identity Card</div>

  <div class="body">
    <div class="photo-row">
      @if($allotment->student->photo ?? null)
        <img src="{{ storage_path('app/public/' . $allotment->student->photo) }}" class="photo" alt="">
      @else
        <div class="photo-placeholder">
          <span>{{ strtoupper(substr($allotment->student->first_name ?? 'S', 0, 1)) }}</span>
        </div>
      @endif
      <div class="info">
        <div class="name">{{ $allotment->student->full_name }}</div>
        <div class="field"><strong>Adm No:</strong> {{ $allotment->student->admission_number }}</div>
        <div class="field"><strong>Class:</strong> {{ $allotment->student->currentClass?->name ?? '—' }}</div>
        <div class="field"><strong>Blood:</strong> {{ $allotment->student->blood_group ?? '—' }}</div>
        <div class="field"><strong>Mobile:</strong> {{ $allotment->student->phone ?? $allotment->student->guardian_mobile ?? '—' }}</div>
      </div>
    </div>
    ...
    <div class="field" style="margin-bottom:4px;">
      <strong>Hostel:</strong> <span class="hostel-badge">{{ $allotment->hostel?->name ?? '—' }}</span>
    </div>
    <div class="field"><strong>Allotted From:</strong> {{ $allotment->allotment_date?->format('d M Y') ?? '—' }}</div>
    @if($allotment->to_date)
    <div class="field"><strong>Valid Until:</strong> {{ $allotment->to_date->format('d M Y') }}</div>
    @endif

    <div class="qr-row">
      <div class="qr-box">{!! $qr !!}</div>
      <div class="room-badge">
        <div class="room-no">{{ $allotment->room?->room_number ?? '—' }}</div>
        <div class="room-label">Room No.</div>
        @if($allotment->bed_number ?? null)
        <div class="room-label" style="margin-top:3px; font-size:8px; color:#1e40af; font-weight:bold;">Bed: {{ $allotment->bed_number }}</div>
        @endif
      </div>
    </div>
  </div>
  ...
  <div class="footer">
    <div class="footer-text">
      {{ $school['school_name'] ?? '' }} &bull; {{ $school['phone'] ?? '' }}<br>
