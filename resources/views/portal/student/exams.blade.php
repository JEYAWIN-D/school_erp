...
<div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: .5rem; margin-bottom: 1rem;">
  ...
  <div style="display: flex; gap: .5rem; flex-wrap: wrap;">
    <a href="{{ route('portal.student.exams.hall-ticket') }}" target="_blank"
    ...
    <a href="{{ route('portal.student.report.pdf') }}" target="_blank"
  ...
  <div style="overflow-x: auto;">
    <table style="width: 100%; border-collapse: collapse; font-size: .875rem;">
      ...
      <tbody>
        ...
        @php $daysLeft = \Carbon\Carbon::parse($up->exam_date)->diffInDays(today(), false); @endphp
        <tr class="divider-row">
          ...
          <td style="padding: .5rem .625rem; text-align: center;">
            <span style="font-weight: 600; color: {{ abs($daysLeft) <= 3 ? '#dc2626' : '#1e293b' }};">{{ \Carbon\Carbon::parse($up->exam_date)->format('d M Y') }}</span>
            @if(abs($daysLeft) == 0) <span class="badge-red" style="margin-left: .25rem;">Today</span>
            @elseif(abs($daysLeft) == 1) <span class="badge-slate" style="margin-left: .25rem;">Tomorrow</span>
            @elseif(abs($daysLeft) <= 3) <span class="badge-red" style="margin-left: .25rem;">{{ abs($daysLeft) }}d</span>
            @else <span style="font-size: .72rem; color: #94a3b8; margin-left: .25rem;">{{ abs($daysLeft) }}d left</span>
            @endif
          </td>
