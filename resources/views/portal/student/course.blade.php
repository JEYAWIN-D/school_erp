...
<div style="margin-bottom: 1rem;">
  <a href="{{ route('portal.student.academics') }}" style="font-size: .8125rem; color: #2563eb; text-decoration: none;">
    &#8592; Back to Academics
  </a>
</div>
...
<div class="portal-card" style="margin-bottom: 1rem;">
  @if($course->thumbnail ?? null)
    <img src="{{ Storage::url($course->thumbnail) }}" style="width:100%;height:12rem;object-fit:cover;border-radius:.75rem;margin-bottom:1rem;">
  @else
  ...
  @endif
  <h1 style="font-size: 1.1875rem; font-weight: 700; color: #1e293b; margin-bottom: .5rem;">{{ $course->title }}</h1>
  @if($course->description ?? null)
    <p style="font-size: .875rem; color: #64748b; line-height: 1.6;">{{ $course->description }}</p>
  @endif
  @if($course->units->count())
  <div style="display:flex;gap:.875rem;margin-top:.875rem;font-size:.75rem;color:#94a3b8;">
    <span>{{ $course->units->count() }} unit(s)</span>
    <span>&bull;</span>
    <span>{{ $course->units->sum(fn($u) => $u->lessons->count()) }} lesson(s)</span>
  </div>
  @endif
</div>
...
<div style="display:flex;flex-direction:column;gap:.75rem;">
  @foreach($course->units->sortBy('sort_order') as $unit)
  <div class="portal-card">
    <div style="font-size:.9375rem;font-weight:700;color:#1e293b;margin-bottom:.625rem;display:flex;align-items:center;gap:.5rem;">
      <span style="width:1.5rem;height:1.5rem;border-radius:50%;background:#ede9fe;color:#7c3aed;font-size:.7rem;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;">{{ $loop->iteration }}</span>
      {{ $unit->title }}
    </div>
    @if($unit->description ?? null)
      <p style="font-size:.8rem;color:#64748b;margin-bottom:.75rem;">{{ $unit->description }}</p>
    @endif
    @if($unit->lessons->count())
    <div style="display:flex;flex-direction:column;gap:.375rem;">
      @foreach($unit->lessons->sortBy('sort_order') as $lesson)
      <div style="display:flex;align-items:center;gap:.75rem;padding:.5rem .75rem;background:#f8fafc;border-radius:.5rem;">
        @php $ctype = $lesson->content_type ?? 'text'; @endphp
        @if($ctype === 'video')
        ...
        @endif
        <span style="font-size:.8rem;color:#334155;flex:1;">{{ $lesson->title }}</span>
        @if($lesson->duration_minutes ?? null)
          <span style="font-size:.72rem;color:#94a3b8;flex-shrink:0;">{{ $lesson->duration_minutes }}min</span>
        @endif
      </div>
      @endforeach
    </div>
