...
<div class="pd-switcher">
  @foreach($children as $c)
    <a href="{{ route('portal.parent.dashboard', ['student_id' => $c->id]) }}"
       style="display:inline-flex;align-items:center;gap:.375rem;padding:.35rem 1rem;border-radius:9999px;font-size:.8125rem;font-weight:600;text-decoration:none;border:1.5px solid {{ $c->id == $child->id ? '#2563eb' : '#e2e8f0' }};background:{{ $c->id == $child->id ? '#2563eb' : '#fff' }};color:{{ $c->id == $child->id ? '#fff' : '#64748b' }};transition:all .15s;">
      {{ $c->first_name }}
    </a>
  @endforeach
</div>
...
{{-- HERO --}}
<div class="pd-hero">
  <div class="pd-avatar">{{ strtoupper(substr($child->first_name ?? '?',0,1)) }}</div>

  <div style="flex:1;min-width:0;position:relative;z-index:1;">
    <p style="color:rgba(255,255,255,.65);font-size:.8rem;margin-bottom:.1rem;">Your child</p>
    <h1 style="color:#fff;font-size:1.3rem;font-weight:800;line-height:1.2;letter-spacing:-.01em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $child->first_name }} {{ $child->last_name }}</h1>
    <div style="display:flex;flex-wrap:wrap;align-items:center;gap:.375rem;margin-top:.4rem;">
      ...
      @endif
      <span style="background:rgba(255,255,255,.08);color:rgba(255,255,255,.75);font-size:.72rem;padding:.2rem .625rem;border-radius:9999px;">{{ $child->admission_no }}</span>
    </div>
  </div>
  ...
  <div style="display:none;flex-direction:column;align-items:flex-end;gap:.5rem;flex-shrink:0;position:relative;z-index:1;" class="pd-hero-meta">
    <span style="background:{{ $attBg }};color:{{ $attFg }};font-size:.8rem;font-weight:700;padding:.3rem .875rem;border-radius:9999px;">● {{ $todayLbl }} Today</span>
    <div style="text-align:right;">
      <p style="color:#fff;font-size:.9rem;font-weight:700;">{{ now()->format('l') }}</p>
      <p style="color:rgba(255,255,255,.55);font-size:.72rem;">{{ now()->format('d M Y') }}</p>
    </div>
  </div>
</div>
...
<div class="pd-stats">
  ...
  <div class="pd-stat">
    ...
    <div style="flex:1;min-width:0;">
      <div class="pd-stat-val">{{ $pct !== null ? $pct.'%' : '—' }}</div>
      <div class="pd-stat-lbl">Attendance</div>
      @if($pct !== null)
        <div style="height:.3rem;border-radius:9999px;background:#e2e8f0;overflow:hidden;margin-top:.4rem;"><div style="height:100%;border-radius:9999px;width:{{ min(100,$pct) }}%;background:{{ $attBar }};"></div></div>
      @endif
    </div>
  </div>

  <div class="pd-stat">
    <div class="pd-stat-icon" style="background:{{ $feeBalance > 0 ? '#fef2f2' : '#f0fdf4' }};">
    ...
    <div style="min-width:0;">
      @if($feeBalance > 0)
        <div class="pd-stat-val" style="color:#dc2626;font-size:1.25rem;">₹{{ number_format($feeBalance) }}</div>
  ...
  <div class="pd-stat">
    ...
    <div>
      <div class="pd-stat-val">{{ $present }}<span style="font-size:.875rem;font-weight:600;color:#94a3b8;">/{{ $totalDays }}</span></div>
  ...
  <div class="pd-stat">
    ...
    <div>
      <div class="pd-stat-val">{{ $hwCount }}</div>
...
<div class="pd-actions">
  <a href="{{ route('portal.parent.attendance') }}" class="pd-action">
  ...
  <a href="{{ route('portal.parent.fees') }}" class="pd-action">
  ...
  <a href="{{ route('portal.parent.exams') }}" class="pd-action">
  ...
  <a href="{{ route('portal.parent.notices') }}" class="pd-action">
...
<div class="pd-grid">
  ...
  <div class="portal-card">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem;margin-bottom:1rem;">
      <div style="background:#f0fdf4;border-radius:.75rem;padding:.875rem .5rem;text-align:center;">
        <div style="font-size:1.5rem;font-weight:800;color:#16a34a;">{{ $present }}</div>
      ...
      <div style="background:#fef2f2;border-radius:.75rem;padding:.875rem .5rem;text-align:center;">
        <div style="font-size:1.5rem;font-weight:800;color:#dc2626;">{{ $absent }}</div>
      ...
      <div style="background:#eff6ff;border-radius:.75rem;padding:.875rem .5rem;text-align:center;">
        <div style="font-size:1.5rem;font-weight:800;color:#2563eb;">{{ $leave }}</div>
    ...
    @if($pct !== null)
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.375rem;">
        ...
        <span style="font-size:.875rem;font-weight:800;color:{{ $attBar }};">{{ $pct }}%</span>
      </div>
      <div style="height:.5rem;border-radius:9999px;background:#e2e8f0;overflow:hidden;">
        <div style="height:100%;border-radius:9999px;width:{{ min(100,$pct) }}%;background:{{ $attBar }};transition:width .5s;"></div>
      </div>
      @if($pct < 75)
  ...
  <div class="portal-card">
    ...
    @forelse($homework as $hw)
      ...
      <div class="divider-row" style="display:flex;align-items:flex-start;gap:.75rem;padding:.625rem 0;">
        ...
        <span style="font-size:.72rem;font-weight:700;color:{{ $urgFg }};background:{{ $urgBg }};padding:.2rem .5rem;border-radius:.375rem;flex-shrink:0;">{{ $urgLbl }}</span>
      </div>
    @empty
  ...
  <div class="portal-card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.875rem;">
      ...
      <a href="{{ route('portal.parent.exams') }}" class="link-sm">Full report</a>
    </div>
    @forelse($recentMarks as $m)
      ...
      <div class="divider-row" style="display:flex;align-items:center;gap:.75rem;padding:.625rem 0;">
        ...
        <div style="text-align:right;flex-shrink:0;">
          ...
          <div style="font-size:.72rem;font-weight:700;color:{{ $pClr }};background:{{ $pBg }};padding:.1rem .375rem;border-radius:.375rem;display:inline-block;">{{ $pct2 }}%</div>
        </div>
      </div>
    @empty
  ...
  <div class="portal-card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.875rem;">
      ...
      <a href="{{ route('portal.parent.notices') }}" class="link-sm">All notices</a>
    </div>
