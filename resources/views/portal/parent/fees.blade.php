...
<div style="margin-bottom: 1rem; display: flex; flex-wrap: wrap; gap: .5rem;">
  @foreach($children as $c)
    <a href="{{ route('portal.parent.fees', ['student_id' => $c->id]) }}"
...
<div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: .5rem; margin-bottom: 1rem;">
  <h2 style="font-size: 1.0625rem; font-weight: 700; color: #1e293b; margin: 0;">Fee Statement — {{ $child->first_name }} {{ $child->last_name }}</h2>
  <a href="{{ route('portal.parent.fees.pdf', ['student_id' => $child->id]) }}" target="_blank"
...
@if($balance > 0)
  <div style="padding: .875rem 1rem; background: #fef2f2; border: 1px solid #fecaca; border-radius: .75rem; color: #dc2626; font-size: .875rem; font-weight: 500; display: flex; align-items: center; gap: .5rem; margin-bottom: 1.25rem;">
    ...
    Outstanding balance of ₹{{ number_format($balance) }}. Please contact the school fee office.
  </div>
@endif
  ...
  <div class="portal-card">
    ...
    <div class="divider-row" style="display: flex; align-items: center; justify-content: space-between; padding: .625rem 0; gap: .5rem;">
      ...
      <div style="text-align: right; flex-shrink: 0;">
        ...
        <a href="{{ route('portal.parent.fees.receipt', array_merge(['id' => $pay->id], request()->only('student_id'))) }}" target="_blank"
