...
@section('title', 'Maintenance Expenses — DASA EduERP')

@section('content')
<div class="space-y-6">

  {{-- Header --}}
  <div style="background:linear-gradient(135deg,#431407 0%,#7c2d12 50%,#c2410c 100%);border-radius:24px;padding:28px 32px;color:#fff;position:relative;overflow:hidden">
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;position:relative">
      <div>
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
          ...
          <span style="padding:3px 10px;border-radius:100px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;background:rgba(253,186,116,0.2);color:#fed7aa;border:1px solid rgba(253,186,116,0.3)">MAINTENANCE EXPENSES</span>
        </div>
        <h1 style="font-size:30px;font-weight:900;color:#fff;letter-spacing:-.02em;margin-bottom:6px">🔧 Maintenance Expenses</h1>
        <p style="font-size:13px;color:rgba(255,255,255,0.65);max-width:540px">Civil works, Electrical systems, Plumbing &amp; RO water, Bus fleet, Campus housekeeping &amp; CCTV security. Approval: <strong style="color:#fed7aa">Admin → Principal → Correspondent</strong></p>
      </div>
      <a href="{{ route('expenses.create') }}?category=maintenance" style="display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.15);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.25);color:#fff;padding:11px 20px;border-radius:14px;font-size:13px;font-weight:700;text-decoration:none;flex-shrink:0">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:20px;position:relative">
      <div style="background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.15);border-radius:14px;padding:16px">
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,0.6);text-transform:uppercase;margin-bottom:4px">Total Approved</div>
  ...
  <div style="background:#fff;border-radius:16px;padding:16px 20px;border:1px solid #e2e8f0;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
    <a href="{{ route('expenses.maintenance') }}" style="padding:6px 14px;border-radius:10px;font-size:12px;font-weight:700;text-decoration:none;{{ !request('subcategory') ? 'background:#f97316;color:#fff' : 'background:#f1f5f9;color:#374151' }}">All</a>
    @foreach($subcategories as $key => $label)
    <a href="{{ route('expenses.maintenance', ['subcategory'=>$key]) }}" style="padding:6px 14px;border-radius:10px;font-size:12px;font-weight:700;text-decoration:none;{{ request('subcategory')===$key ? 'background:#f97316;color:#fff' : 'background:#f1f5f9;color:#374151' }}">{{ $label }}</a>
    @endforeach
  </div>
  ...
  <div style="background:#fff;border-radius:20px;border:1px solid #e2e8f0;overflow:hidden">
    <div style="padding:20px 24px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
      <span style="font-size:14px;font-weight:700;color:#1e293b">Maintenance Vouchers <span style="color:#64748b;font-weight:500">({{ $expenses->total() }})</span></span>
      <form method="GET" style="display:flex;align-items:center;gap:8px">
        ...
        <button type="submit" style="padding:7px 14px;background:#f97316;color:#fff;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer">Filter</button>
      </form>
    </div>

    {{-- Approval Chain --}}
    <div style="padding:14px 24px;background:#fff7ed;border-bottom:1px solid #fed7aa;display:flex;align-items:center;gap:8px;font-size:12px">
      <span style="font-weight:600;color:#7c2d12">Approval Chain:</span>
      @foreach(['Admin (Prepares)','Principal (Verifies)','Correspondent / School Owner (Approves)'] as $i => $step)
        @if($i > 0)<span style="color:#fb923c">→</span>@endif
        <span style="padding:3px 10px;background:#fed7aa;color:#7c2d12;border-radius:8px;font-weight:700">{{ $step }}</span>
      @endforeach
    </div>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        <thead>
          <tr style="background:#f8fafc">
            ...
            <th style="text-align:left;padding:12px 16px;font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.04em">Facility Category</th>
            <th style="text-align:left;padding:12px 16px;font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.04em">Description &amp; Vendor</th>
        ...
        <tbody>
          ...
          <tr style="border-top:1px solid #f1f5f9" onmouseover="this.style.background='#fafaf9'" onmouseout="this.style.background=''">
            <td style="padding:14px 20px">
              <span style="font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:700;color:#c2410c;background:#fff7ed;padding:3px 8px;border-radius:6px">{{ $expense->expense_number }}</span>
            </td>
            ...
            <td style="padding:14px 20px">
              <div style="display:flex;align-items:center;gap:8px">
                <a href="{{ route('expenses.show', $expense->id) }}" style="font-size:12px;font-weight:700;color:#f97316;text-decoration:none;padding:5px 10px;border:1px solid #fed7aa;border-radius:8px">View</a>
          ...
          <tr><td colspan="7" style="padding:48px;text-align:center;color:#94a3b8">
            ...
            <div style="font-weight:600">No maintenance expenses found</div>
            <a href="{{ route('expenses.create') }}?category=maintenance" style="color:#f97316;font-size:13px;font-weight:600;text-decoration:none">+ Add first maintenance expense</a>
          </td></tr>
          @endforelse
        </tbody>
      </table>
