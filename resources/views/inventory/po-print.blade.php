...
<div class="meta">
  <div class="meta-block">
    ...
    @if($po->delivery_date)
    <p><strong>Delivery By:</strong> {{ \Carbon\Carbon::parse($po->delivery_date)->format('d M Y') }}</p>
    @endif
    <p><strong>Status:</strong> <span class="badge badge-{{ $po->status }}">{{ ucfirst($po->status) }}</span></p>
  </div>
  <div class="meta-block">
    @if($po->payment_terms)
    ...
    <p><strong>Ship To:</strong> {{ $po->shipping_address }}</p>
    @endif
  </div>
</div>
...
<div class="vendor-box">
  ...
  <p><strong>Name:</strong> {{ $po->vendor?->name ?? '—' }}</p>
  @if($po->vendor?->email)<p><strong>Email:</strong> {{ $po->vendor->email }}</p>@endif
  @if($po->vendor?->phone)<p><strong>Phone:</strong> {{ $po->vendor->phone }}</p>@endif
  @if($po->vendor?->address)<p><strong>Address:</strong> {{ $po->vendor->address }}</p>@endif
  @if($po->vendor?->gstin)<p><strong>GSTIN:</strong> {{ $po->vendor->gstin }}</p>@endif
</div>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>{{ $i + 1 }}</td>
      <td>{{ $line->inventoryItem?->name ?? $line->description ?? '—' }}</td>
      <td style="text-align:center">{{ $line->quantity }}</td>
      <td style="text-align:right">{{ $line->unit_price ? number_format($line->unit_price, 2) : '—' }}</td>
...
<div style="margin-top:24px;text-align:right">
  ...
  <a href="{{ route('inventory.purchase-orders') }}" class="back-btn" style="margin-left:8px;padding:8px 20px;background:#f1f5f9;color:#1e293b;border:none;border-radius:6px;cursor:pointer;text-decoration:none;font-size:13px;">← Back</a>
</div>
</body>
</html>
