...
@section('content')
<div class="space-y-6" x-data="{ showForm: {{ old('name') ? 'true' : 'false' }}, editId: null, editData: {} }">
  ...
  <div x-show="showForm" x-collapse class="card">
    ...
    <form method="POST" action="{{ route('inventory.vendors.store') }}" class="grid grid-cols-2 gap-4">
      @csrf
  ...
  <form method="GET" class="flex gap-3">
    <input type="text" name="search" value="{{ request('search') }}" class="input max-w-xs" placeholder="Search name or phone…">
  ...
  <div class="table-wrap">
    <table class="w-full text-sm">
      ...
      <tbody>
        ...
        <tr class="tr">
          <td class="td"><p class="font-medium">{{ $v->name }}</p><p class="text-xs text-slate-400">{{ $v->code }}</p></td>
          <td class="td">{{ $v->contact_person ?? '—' }}</td>
          <td class="td">{{ $v->phone ?? '—' }}</td>
          <td class="td text-xs">{{ $v->email ?? '—' }}</td>
          <td class="td font-mono text-xs">{{ $v->gstin ?? '—' }}</td>
          <td class="td">@if($v->is_active) <span class="badge-green">Active</span> @else <span class="badge-slate">Inactive</span> @endif</td>
          <td class="td">
            <button @click="editId={{ $v->id }};editData={{ json_encode($v) }};showForm=false" class="btn-xs btn-secondary">Edit</button>
          </td>
        </tr>
        @empty
  ...
  <div>{{ $vendors->withQueryString()->links() }}</div>
