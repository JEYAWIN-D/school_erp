...
<div class="space-y-5">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    ...
    <div class="flex gap-2 flex-wrap">
      <a href="{{ route('fees.structure') }}" class="btn btn-secondary btn-sm">Fee Structure</a>
      <a href="{{ route('fees.defaulters') }}" class="btn btn-secondary btn-sm">
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="card">
      ...
      <div class="mt-3">
        <p class="stat-number text-emerald-700">₹{{ number_format($todayCollection) }}</p>
    ...
    <div class="card">
      ...
      <div class="mt-3">
        <p class="stat-number">₹{{ number_format($monthCollection) }}</p>
    ...
    <div class="card">
      ...
      <div class="mt-3">
        <p class="stat-number">₹{{ number_format($yearCollection) }}</p>
        <p class="text-xs text-slate-500 mt-1 font-medium">Year Collected</p>
      </div>
    </div>

    <a href="{{ route('fees.defaulters') }}" class="card hover:shadow-md transition-shadow group">
      <div class="flex items-start justify-between">
      ...
      <div class="mt-3">
        <p class="stat-number text-red-700">{{ number_format($defaultersCount) }}</p>
  ...
  <div class="card">
    <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
      <div>
        ...
        <p class="text-xs text-slate-400 mt-0.5">Total billed vs collected &mdash; {{ $currentYear?->name }}</p>
      </div>
      <div class="text-right">
        <p class="text-lg font-bold text-slate-900" style="font-family:'Plus Jakarta Sans',sans-serif;letter-spacing:-0.03em">
          {{ $yearDemand > 0 ? round($yearCollection / $yearDemand * 100, 1) : 0 }}%
        </p>
        <p class="text-xs text-slate-400 mt-0.5">₹{{ number_format($yearCollection) }} of ₹{{ number_format($yearDemand) }}</p>
      </div>
    </div>
    <div class="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
      <div class="h-2.5 rounded-full bg-emerald-500 transition-all duration-500"
           style="width: {{ min(100, $yearDemand > 0 ? round($yearCollection / $yearDemand * 100, 1) : 0) }}%"></div>
    </div>
    <div class="flex justify-between mt-2.5 text-xs font-semibold">
      <span class="text-emerald-600">Collected: ₹{{ number_format($yearCollection) }}</span>
      @if($outstanding > 0)
        <span class="text-red-600">Outstanding: ₹{{ number_format($outstanding) }}</span>
      @else
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
    ...
    <a href="{{ route($route) }}"
       class="card-flat flex items-center gap-3 py-3 px-3.5 border hover:shadow-sm transition-all hover:border-slate-300 group {{ $color }}">
    ...
    <div class="table-wrap">
      <table class="w-full">
        ...
        <tbody>
          ...
          <tr class="tr">
            <td class="td font-mono text-xs font-semibold text-indigo-600">{{ $p->receipt_number }}</td>
            <td class="td font-semibold text-slate-800">{{ $p->student?->full_name }}</td>
            <td class="td text-slate-500">{{ $p->feeHead?->name ?? '—' }}</td>
            <td class="td font-bold text-emerald-700">₹{{ number_format($p->total_paid, 2) }}</td>
            <td class="td">
              <span class="badge-slate capitalize">{{ $p->payment_mode ?? 'cash' }}</span>
            </td>
            <td class="td text-slate-400 text-xs">{{ \Carbon\Carbon::parse($p->payment_date)->format('d M Y') }}</td>
            <td class="td text-right">
              <a href="{{ route('fees.receipt', $p->id) }}" target="_blank" class="btn-icon text-indigo-500 hover:text-indigo-700 hover:bg-indigo-50" title="Print receipt">
