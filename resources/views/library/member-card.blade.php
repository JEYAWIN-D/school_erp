...
<body class="bg-white p-4">
  <div class="no-print mb-4 flex gap-2">
    ...
    <a href="{{ route('library.members') }}" class="btn btn-secondary btn-sm">Back</a>
  </div>
  <div class="card-box">
    <div class="flex items-center gap-2 mb-2">
      ...
      <div>
        <p class="font-bold text-slate-800 text-sm leading-tight">{{ $student->full_name }}</p>
        <p class="text-xs text-slate-500">{{ $student->currentEnrollment?->class?->name ?? '' }}</p>
      </div>
    </div>
    <div class="grid grid-cols-2 gap-1 text-xs text-slate-600">
      <div><span class="text-slate-400">Adm No:</span> {{ $student->admission_number }}</div>
      <div><span class="text-slate-400">Year:</span> {{ $student->currentEnrollment?->academicYear?->name ?? date('Y') }}</div>
    </div>
    <div class="mt-2 pt-2 border-t border-indigo-100 flex items-center justify-between">
      ...
      <p class="text-xs text-slate-400">Valid: {{ date('Y') }}-{{ date('y', strtotime('+1 year')) }}</p>
    </div>
  </div>
</body>
</html>
