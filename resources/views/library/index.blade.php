...
<div class="space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <p class="page-subtitle">{{ $stats['total_books'] }} book titles · {{ $stats['total_copies'] }} total copies</p>
    </div>
    <div class="flex gap-2 flex-wrap">
      ...
      <a href="{{ route('library.books.create') }}" class="btn btn-secondary btn-sm">Add Book</a>
      <a href="{{ route('library.issue') }}" class="btn btn-primary btn-sm">
  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
    <a href="{{ route('library.books') }}" class="card text-center py-5 hover:shadow-card-md transition">
      <p class="text-3xl font-bold text-slate-800" style="font-family:'Plus Jakarta Sans',sans-serif;">{{ $stats['total_copies'] }}</p>
      <p class="text-sm text-slate-500 mt-1">Total Copies</p>
    </a>
    <a href="{{ route('library.books', ['available' => 1]) }}" class="card text-center py-5 hover:shadow-card-md transition">
      <p class="text-3xl font-bold text-green-600" style="font-family:'Plus Jakarta Sans',sans-serif;">{{ $stats['available'] }}</p>
      <p class="text-sm text-slate-500 mt-1">Available</p>
    </a>
    <a href="{{ route('library.currently-issued') }}" class="card text-center py-5 hover:shadow-card-md transition">
      <p class="text-3xl font-bold text-blue-600" style="font-family:'Plus Jakarta Sans',sans-serif;">{{ $stats['issued'] }}</p>
      <p class="text-sm text-slate-500 mt-1">Issued</p>
    </a>
    <a href="{{ route('library.overdue-report') }}" class="card text-center py-5 hover:shadow-card-md transition">
      <p class="text-3xl font-bold {{ $stats['overdue'] > 0 ? 'text-red-500' : 'text-slate-800' }}" style="font-family:'Plus Jakarta Sans',sans-serif;">{{ $stats['overdue'] }}</p>
  ...
  <div class="rounded-xl border border-red-200 bg-red-50 p-4">
    <div class="flex items-start gap-3 mb-3">
      ...
      <div class="flex-1">
        <p class="font-semibold text-red-800">{{ $stats['overdue'] }} overdue book{{ $stats['overdue'] > 1 ? 's' : '' }}
          @if($totalFinesPending > 0)
          — ₹{{ number_format($totalFinesPending, 2) }} in pending fines
          @endif
        </p>
        @if($overdueIssues->count())
        <div class="mt-2 space-y-1">
          @foreach($overdueIssues as $oi)
          <div class="flex items-center justify-between text-sm">
            <span class="text-red-700">{{ $oi->book?->title ?? '—' }}</span>
            <span class="text-red-500 text-xs font-medium">{{ $oi->student?->full_name ?? $oi->borrowerName() }} · Due {{ \Carbon\Carbon::parse($oi->due_date)->format('d M') }}</span>
          </div>
          @endforeach
          @if($stats['overdue'] > $overdueIssues->count())
          <a href="{{ route('library.overdue-report') }}" class="text-xs text-red-600 hover:underline">+{{ $stats['overdue'] - $overdueIssues->count() }} more — view full report</a>
          @endif
        </div>
        @endif
      </div>
      <a href="{{ route('library.overdue-report') }}" class="btn btn-sm bg-red-600 hover:bg-red-700 text-white flex-shrink-0">Full Report</a>
    </div>
  </div>
  @endif

  {{-- Quick Book Search --}}
  <form method="GET" action="{{ route('library.books') }}" class="card-flat py-4">
    <div class="flex gap-2">
      ...
      <a href="{{ route('library.books', ['available' => 1]) }}" class="btn btn-secondary btn-sm">Available Only</a>
    </div>
  </form>
  ...
  <div class="card">
    <div class="flex items-center justify-between mb-4">
      ...
      <a href="{{ route('library.currently-issued') }}" class="text-sm text-blue-600 hover:underline">View All →</a>
    </div>
      ...
      <table class="w-full">
        ...
        <tbody>
          @forelse($recentIssues as $issue)
            <tr class="tr">
              <td class="td">
                <p class="font-medium text-slate-800">{{ $issue->book?->title ?? '—' }}</p>
                @if($issue->book?->author)
                <p class="text-xs text-slate-400">{{ $issue->book->author }}</p>
                @endif
              </td>
              <td class="td">{{ $issue->borrowerName() }}</td>
              <td class="td text-sm">{{ \Carbon\Carbon::parse($issue->issue_date)->format('d M Y') }}</td>
              <td class="td text-sm {{ $issue->status === 'issued' && \Carbon\Carbon::parse($issue->due_date)->isPast() ? 'text-red-600 font-semibold' : 'text-slate-600' }}">
