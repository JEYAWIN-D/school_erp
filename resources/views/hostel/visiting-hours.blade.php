@extends('layouts.app')
@section('title', 'Visiting Hours Configuration')
@section('content')
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <h1 class="page-title">Visiting Hours Configuration</h1>
    <a href="{{ route('hostel.visitors') }}" class="btn-secondary btn-sm">← Back to Visitors</a>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="card">
      ...
      <form method="POST" action="{{ route('hostel.visiting-hours.store') }}" class="space-y-4">
        @csrf
        <div>
    ...
    <div class="lg:col-span-2 card">
      ...
      <div class="table-wrap">
        <table class="w-full">
          ...
          <tbody>
            ...
            <tr class="tr">
              <td class="td">{{ $rule->hostel?->name ?? 'All Hostels' }}</td>
              <td class="td"><span class="badge-blue capitalize">{{ str_replace('_', ' ', $rule->day_type) }}</span></td>
              <td class="td font-mono">{{ $rule->from_time }}</td>
              <td class="td font-mono">{{ $rule->to_time }}</td>
              <td class="td text-slate-400 text-xs">{{ $rule->note ?? '—' }}</td>
              <td class="td">
                <form method="POST" action="{{ route('hostel.visiting-hours.delete', $rule->id) }}" class="inline">
                  @csrf @method('DELETE')
      ...
      <div class="mt-4 bg-slate-50 rounded-lg p-4">
        ...
        @php $now = now(); $allowed = \App\Models\HostelVisitingHours::isCurrentlyAllowed(); @endphp
        <div class="flex items-center gap-3">
          <span class="badge-{{ $allowed ? 'green' : 'red' }}">{{ $allowed ? 'Visiting Allowed' : 'Outside Hours' }}</span>
          <span class="text-sm text-slate-500">Current time: {{ $now->format('h:i A') }} ({{ $now->isWeekend() ? 'Weekend' : 'Weekday' }})</span>
        </div>
      </div>
    </div>
  </div>
