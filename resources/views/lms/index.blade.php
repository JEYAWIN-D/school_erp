@extends('layouts.app')
@section('title', 'LMS — Online Learning')
@section('content')
<div class="space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      ...
      <p class="page-subtitle">{{ $publishedCount }} published courses &bull; {{ $totalLessons }} lessons</p>
    </div>
    <a href="{{ route('lms.courses.create') }}" class="btn btn-primary self-start sm:self-auto">
  ...
  <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 flex items-center gap-3">
    ...
    <p class="text-sm font-semibold text-amber-800">{{ $pendingSubmissions }} assignment submission(s) awaiting grading.</p>
  </div>
  @endif
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
    ...
    <div class="card">
      ...
      <p class="stat-number text-green-600">{{ $publishedCount }}</p>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
    ...
    <div class="card hover:shadow-md transition group">
      @if($course->thumbnail)
        <img src="{{ Storage::url($course->thumbnail) }}" class="w-full h-32 object-cover rounded-xl mb-3 -mt-1 -mx-1">
      @else
      ...
      <div class="flex items-start justify-between gap-2">
        <h3 class="font-semibold text-slate-800 text-sm leading-tight">{{ $course->title }}</h3>
        <span class="badge-{{ $course->status === 'published' ? 'green' : ($course->status === 'archived' ? 'red' : 'blue') }} flex-shrink-0 text-xs">
          {{ ucfirst($course->status) }}
        </span>
      </div>
      <p class="text-xs text-slate-400 mt-1">{{ $course->units_count }} units &bull; {{ $course->lessons_count }} lessons</p>
      <p class="text-xs text-slate-400">By {{ $course->creator?->name ?? '—' }}</p>
      <div class="flex gap-2 mt-3">
        <a href="{{ route('lms.courses.show', $course->id) }}" class="btn-xs btn-primary flex-1 text-center">Manage</a>
        <a href="{{ route('lms.courses.progress', $course->id) }}" class="btn-xs btn-secondary flex-1 text-center">Progress</a>
      </div>
    </div>
    @empty
    <div class="col-span-full card text-center py-12">
      <p class="text-slate-400 mb-3">No courses created yet</p>
      <a href="{{ route('lms.courses.create') }}" class="btn-primary btn-sm">Create First Course</a>
    </div>
    @endforelse
  </div>
