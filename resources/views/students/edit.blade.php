...
<div class="max-w-4xl mx-auto space-y-6">
  ...
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="page-subtitle font-mono">{{ $student->admission_number }}</p>
    </div>
  </div>
      ...
      <div class="flex-shrink-0">
        <div class="w-28 h-32 rounded-lg border-2 border-dashed border-slate-300 overflow-hidden bg-slate-50 flex items-center justify-center">
          @if($student->photo && Storage::disk('public')->exists($student->photo))
            <img id="current-photo" src="{{ Storage::url($student->photo) }}?v={{ time() }}" class="w-full h-full object-cover">
          @else
    ...
    <div class="card">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          ...
          <input type="text" name="first_name" value="{{ old('first_name', $student->first_name) }}" class="input @error('first_name') input-error @enderror">
        </div>
        <div>
          <label class="label">Last Name <span class="text-red-500">*</span></label>
          <input type="text" name="last_name" value="{{ old('last_name', $student->last_name) }}" class="input">
        </div>
        <div>
          <label class="label">Date of Birth <span class="text-red-500">*</span></label>
          <input type="date" name="dob" value="{{ old('dob', $student->dob?->toDateString()) }}" class="input">
        </div>
        <div>
        ...
        <div>
          ...
          <select name="category" class="select">
            <option value="">Select</option>
            @foreach(['general' => 'General', 'obc' => 'OBC', 'sc' => 'SC', 'st' => 'ST', 'ews' => 'EWS'] as $val => $lbl)
              <option value="{{ $val }}" @selected(old('category', $student->category) === $val)>{{ $lbl }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div>
          ...
          <input type="tel" name="mobile" value="{{ old('mobile', $student->mobile) }}" class="input">
        </div>
        <div>
        ...
        <div class="sm:col-span-3" x-data="{ copyAddr() { $el.querySelector('[name=permanent_address]').value = $el.querySelector('[name=residential_address]').value } }">
          ...
          <input type="text" name="residential_address" id="resAddr"
              value="{{ old('residential_address', $student->residential_address) }}" class="input">
        </div>
        <div class="sm:col-span-3" x-data="{}">
          ...
          <input type="text" name="permanent_address"
              value="{{ old('permanent_address', $student->permanent_address) }}" class="input">
        </div>
        <div class="sm:col-span-3" x-data="{ disabled: {{ $student->is_disabled ? 'true' : 'false' }} }">
          <div class="flex items-center gap-3 mt-2">
            <input type="checkbox" name="is_disabled" value="1" id="isDisabled" class="rounded w-4 h-4 text-indigo-600"
                x-model="disabled" @checked($student->is_disabled)>
          ...
          <div x-show="disabled" x-transition class="mt-2">
            <input type="text" name="disability_description" class="input"
                value="{{ old('disability_description', $student->disability_description) }}"
    ...
    <div class="card">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="father_name" value="{{ old('father_name', $student->father_name) }}" class="input">
        </div>
        <div>
          <label class="label">Father's Mobile</label>
          <input type="tel" name="father_mobile" value="{{ old('father_mobile', $student->father_mobile) }}" class="input">
        </div>
        <div>
          <label class="label">Mother's Name <span class="text-red-500">*</span></label>
          <input type="text" name="mother_name" value="{{ old('mother_name', $student->mother_name) }}" class="input">
        </div>
        <div>
          <label class="label">Mother's Mobile</label>
          <input type="tel" name="mother_mobile" value="{{ old('mother_mobile', $student->mother_mobile) }}" class="input">
        </div>
        <div>
          <label class="label">Annual Family Income (₹)</label>
          <input type="number" name="annual_family_income" min="0" step="1000"
              value="{{ old('annual_family_income', $student->annual_family_income) }}" class="input"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          ...
          <select name="student_type" class="select">
            <option value="day_scholar"  @selected(old('student_type', $student->student_type) === 'day_scholar')>Day Scholar</option>
            <option value="hosteller"    @selected(old('student_type', $student->student_type) === 'hosteller')>Hosteller</option>
            <option value="day_boarder"  @selected(old('student_type', $student->student_type) === 'day_boarder')>Day Boarder</option>
          </select>
        </div>
        <div>
          <label class="label">House</label>
          <input type="text" name="house"
              value="{{ old('house', $student->currentEnrollment?->house) }}" class="input"
        ...
        <div>
          ...
          <select name="parent_employee_id" class="select">
            ...
            @foreach($employees as $emp)
              <option value="{{ $emp->id }}" @selected(old('parent_employee_id', $student->parent_employee_id) == $emp->id)>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="previous_school_name"
              value="{{ old('previous_school_name', $student->previous_school_name) }}" class="input">
        </div>
        <div>
          <label class="label">Previous School Board</label>
          <input type="text" name="previous_school_board"
              value="{{ old('previous_school_board', $student->previous_school_board) }}" class="input"
              placeholder="e.g. CBSE, TNBSE">
        </div>
        <div>
          <label class="label">TC Number</label>
          <input type="text" name="tc_number"
              value="{{ old('tc_number', $student->tc_number) }}" class="input">
        </div>
        <div>
          <label class="label">TC Date</label>
          <input type="date" name="tc_date"
              value="{{ old('tc_date', $student->tc_date?->toDateString()) }}" class="input">
        </div>
        <div>
          ...
          <input type="number" name="previous_percentage" step="0.01" min="0" max="100"
              value="{{ old('previous_percentage', $student->previous_percentage) }}" class="input"
              placeholder="e.g. 85.5">
        </div>
        <div>
          <label class="label">Migration Certificate Number</label>
          <input type="text" name="migration_certificate_number"
              value="{{ old('migration_certificate_number', $student->migration_certificate_number) }}" class="input">
        </div>
        <div>
          <label class="label">Migration Certificate Date</label>
          <input type="date" name="migration_certificate_date"
              value="{{ old('migration_certificate_date', $student->migration_certificate_date?->toDateString()) }}" class="input">
        </div>
      </div>
    </div>
...
function photoUploader() {
  return {
    ...
    uploading: false,
    cropper: null,
    uploadUrl: '{{ route("students.photo.upload", $student->id) }}',
